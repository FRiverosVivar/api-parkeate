...
export class VehicleService {
  ...
  createVehicle(createVehicleInput: CreateVehicleInput, ownerId: string): Observable<VehicleEntity> {
    const vehicle = this.vehicleRepository.create(createVehicleInput);
    const user = this.userServices.findUserById(ownerId);
    return user.pipe(
      switchMap((u) => {
        vehicle.owner = u;
        return this.getVehicleByPlate(vehicle.carPlate)
          .pipe(
            switchMap((v) => {
              if(v) {
  ...
  updateVehicle(updateVehicleInput: UpdateVehicleInput): Observable<VehicleEntity> {
    return from(
  ...
  removeVehicle(vehicleId: string): Observable<VehicleEntity> {
    ...
    return from(
      ...
      switchMap((vehicle) => {
        return from(this.vehicleRepository.remove([vehicle])).pipe(map((v) => v[0]));
      }),
    );
  }
  ...
  private getVehicleById(vehicleId: string) : Observable<VehicleEntity | null> {
    return from(
  ...
  private getVehiclesByUserId(userId: string) : Observable<VehicleEntity[] | null> {
    return from(
