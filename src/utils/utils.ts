...
} from "src/liquidation/model/liquidation-template.input";
...
import { BookingDataEmailOutput } from "src/booking/model/booking-data-email.output";
...
export async function readPdfTemplateFromFilesAndCompileWithData(
  data: LiquidationPDFTemplate
) {
  let templateHtml = readFileSync(
    path.resolve(__dirname + "/liquidation/liquidation.template.html")
  );
...
export function generateLiquidationTemplateDataToFulfillPdfTemplate(
...
export function mapBookingsToPDFTemplateBookings(
...
export function getBookingDataForNewBookingEmailTemplate(
  ...
  const output = {
    name: parking.client.fullname,
    bookingType: BookingTypeNameEnum[booking.bookingType],
    buildingName: building.name,
    buildingAddress: building.address,
    buildingPhone: building.phoneNumber,
    parkingNumber: parking.code,
    parkingFloor: parking.floor.toString(),
    parkingSector: parking.section,
    clientName: user.fullname,
    clientRut: user.rut,
    clientVehiclePlate: vehicle.carPlate,
    clientVehicleModel: vehicle.model,
  };
  console.log(output);
  return output;
}
